...
export async function getListsInFolder(folderId: string) {
  const data = await clickup<ClickUpListsResponse>(
    `/folder/${clickupPathId(folderId)}/list?archived=false`,
  );
  return data.lists;
}
...
export async function getFolderlessLists(spaceId: string) {
  const data = await clickup<ClickUpListsResponse>(
    `/space/${clickupPathId(spaceId)}/list?archived=false`,
  );
  return data.lists;
}

export async function getList(listId: string) {
  return clickup<ClickUpListResponse>(`/list/${clickupPathId(listId)}`);
}
